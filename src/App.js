...
const TodoItem = ({ todo, onEdit, onDelete, onModify, onAssign }) => {
  const handleDue = () => {
    ...
    if (newDueDate) {
      onModify(todo.todoText, newDueDate);
    }
  };
      ...
      <div className="d-flex align-items-center">
        <div className="btn-group btn-group-sm" role="group">
          ...
          <select
            ...
            value={todo.priority}
            onChange={(e) => handlePriority(e, todo.todoText)}
          >
...
const App = () => {
  ...
  const addTodo = (todoText, dueDate, priority) => {
    if (todoText.trim() !== "") {
      const newTodo = { todoText, dueDate, priority };
  ...
  const editTodo = (oldTodoText, newTodoText) => {
    const newTodos = todos.map((todo) =>
      todo.todoText === oldTodoText ? { ...todo, todoText: newTodoText } : todo
    );
    setTodos(newTodos);
  };

  const modifyDate = (todoText, newDate) => {
    setTodos(todos.map(todo => 
        todo.todoText === todoText ? { ...todo, dueDate: newDate } : todo
    ));
};

  const assignPriority = (todoText, newPriority) => {
    const newTodos = todos.map((todo) =>
      todo.todoText === todoText ? { ...todo, priority: newPriority } : todo
    );
    setTodos(newTodos);
  };
  ...
  const handleSearchSubmit = (event) => {
    ...
    const results = todos.filter((todo) =>
      todo.todoText.toLowerCase().includes(searchTerm.toLowerCase())
    );
          ...
          <Form id="todo-form" className="mb-3" onSubmit={handleFormSubmit}>
            <Row>
              <InputGroup className="mb-3">
                ...
                <Col xs={2}>
                  <FormLabel htmlFor="due-date-input">
                    Due Date (Optional)
                  </FormLabel>
                  <FormControl
                    type="date"
